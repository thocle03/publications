...
%-------------------------------------------------------------------------------
\section{Conclusion}
This paper presented a Topological Graph-Spectral Machine Learning framework for fast surrogate prediction of simulation-derived urban $\text{CO}_2$ emissions. By modeling road networks as asymmetric physically weighted impedance matrices and extracting non-normal operator signatures—including spectral radius $\rho(A)$, Kreiss constants $K(A)$, Schur non-normalness $\Delta(A)$, Hardy norms ($H_2, H_\infty$), and Kato perturbation derivatives—we successfully bypassed the severe computational limitations of traditional microscopic multi-agent physics simulators. 
