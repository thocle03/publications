...
Table~\ref{tab:features_all} provides the complete mathematical and physical inventory of all 47 features.

\begin{table*}[htbp]
\caption{Complete Inventory of 47 Topological, Spectral, Demand, and Electrification Descriptors}
\label{tab:features_all}
\centering
\begin{tabularx}{\textwidth}{llclX}
\toprule
\textbf{Family} & \textbf{Descriptor Name} & \textbf{Dimension} & \textbf{Mathematical Definition / Formula} & \textbf{Physical \& Traffic Significance} \\
\midrule
\multirow{4}{*}{\textbf{1. Demand}} 
& Total Vehicles $N_{\text{veh}}$ & 1 & $\sum_k v_k$ (total simulated agents) & Global volume scaling baseline \\
& Mean Duration $\bar{T}_{\text{sim}}$ & 1 & $\frac{1}{N} \sum_k (t_{\text{arr}, k} - t_{\text{dep}, k})$ (seconds) & Average network dwell time \\
& Total Trajectory Length $L_{\text{tot}}$ & 1 & $\sum_k \int_0^{T_k} \| \mathbf{v}_k(t) \| dt$ (meters) & Total vehicle-kilometers traveled (VKT) \\
& Mean Free Speed $\bar{v}_{\text{free}}$ & 1 & $\frac{1}{m} \sum_{e \in E} W_e$ (m/s) & Average legal network speed limit \\
\midrule
\multirow{2}{*}{\textbf{2. Fleet}} 
& Electrification Ratio $\text{Fleet}_{\text{elec}}$ & 1 & $N_{\text{EV}} / N_{\text{veh}} \in [0, 1]$ & Zero-tailpipe emission fraction \\
& Heavy Duty Fraction $\text{Fleet}_{\text{HGV}}$ & 1 & $N_{\text{HGV}} / N_{\text{veh}} \in [0, 1]$ & High-emission freight vehicle fraction \\
\midrule
\multirow{8}{*}{\textbf{3. Topology}} 
& Node Count $n$, Edge Count $m$ & 2 & $|V|, |E|$ & Raw topological graph size \\
& Network Density $\delta$ & 1 & $m / (n(n-1))$ & Global link interconnectivity \\
& Mean Degree $\bar{k}$, Std $\sigma_k$ & 2 & $\frac{1}{n} \sum_i k_i$, $\sqrt{\frac{1}{n} \sum_i (k_i - \bar{k})^2}$ & Degree distribution moments \\
& Mean Length $\bar{L}$, Total Length $L_{\text{net}}$ & 2 & $\frac{1}{m} \sum_{ij} L_{ij}$, $\sum_{ij} L_{ij}$ & Physical road network footprint \\
& Speed Limit Variance $\sigma_W^2$ & 1 & $\frac{1}{m} \sum_{ij} (W_{ij} - \bar{W})^2$ & Kinetic speed heterogeneity \\
& Total Lane Capacity $C_{\text{tot}}$ & 1 & $\sum_{ij} C_{ij}$ & Total spatial vehicle storage capacity \\
& Circuity $\chi$ & 1 & $\frac{1}{m} \sum_{ij} L_{ij} / d_{\text{Euclid}}(i, j)$ & Detour factor over straight lines \\
& Meshedness Coefficient $M$ & 1 & $(m - n + 1) / (2n - 5)$ & Planar face density and grid meshing \\
\midrule
\multirow{18}{*}{\textbf{4. Spectral}} 
& Spectral Radius $\rho(A_0), \rho(A_w)$ & 2 & $\max_i |\lambda_i(A_0)|, \max_i |\lambda_i(A_w)|$ & Structural connectivity and impedance capacity \\
& Spectral Gap $\Delta\lambda$ & 1 & $|\lambda_1(\tilde{A}_0) - \lambda_2(\tilde{A}_0)|$ (descending modulus) & Mixing time and alternative routing redundancy \\
& Top-5 Eigenvalue Moduli & 5 & $|\lambda_1(\tilde{A}_w)|, \dots, |\lambda_5(\tilde{A}_w)|$ & Multiscale relaxation modes of $\tilde{A}_w$ (4) \\
& Commutator Non-Normality & 2 & $\Delta(\tilde{A}_0), \Delta(\tilde{A}_w)$ from \eqref{eq:commutator} & Asymmetric vorticity and directional friction \\
& Kreiss Constant $K(\tilde{A}_0), K(\tilde{A}_w)$ & 2 & $\sup_{|z|>1} (|z|-1)\|(zI - \tilde{A})^{-1}\|$ & Transient congestion shockwave growth peak \\
& Hardy Resolvent Norm & 2 & $\|R(z, \tilde{A}_0)\|_{H_2}, \|R(z, \tilde{A}_w)\|_{H_2}$ & Integrated dynamic perturbation sensitivity \\
& Eigenspectrum Derivatives & 4 & First/second derivatives $\lambda^{(1)}, \lambda^{(2)}$ of (10) & Kato spectral sensitivity under link perturbation \\
\midrule
\multirow{9}{*}{\textbf{5. Cross}} 
& Demand Density $N_{\text{veh}} / L_{\text{net}}$ & 1 & Total vehicles per network meter & Macro spatial traffic load \\
& Capacity Saturation $N_{\text{veh}} / C_{\text{tot}}$ & 1 & Vehicles per available lane-meter & Global bottleneck vulnerability index \\
& Spectral Shock Index & 1 & $N_{\text{veh}} \cdot K(\tilde{A}_w) / C_{\text{tot}}$ & Combined non-linear shockwave risk \\
& Thermal Emission Factor & 1 & $N_{\text{veh}} \cdot (1 - \text{Fleet}_{\text{elec}})$ & Net ICE vehicle volume \\
& Non-Normal Shearing Load & 1 & $N_{\text{veh}} \cdot \Delta(\tilde{A}_0) / n$ & Asymmetry-driven intersection queuing stress \\
& Kinetic Dissipation Index & 4 & Cross-ratios of $\rho(A_w), \sigma_W, \bar{T}_{\text{sim}}$ & Non-linear kinematic kinetic losses \\
\bottomrule
\end{tabularx}
\end{table*}

\textit{Analytical Commentary on the 47 Descriptors}:
As structured in Table~\ref{tab:features_all}, each descriptor family addresses a specific scale of the urban mobility process. Family 1 (Demand) provides macroscopic boundary conditions, while Family 2 (Fleet) establishes the stoichiometric baseline of combustion vs electric powertrains. Family 3 (Topology) captures static Euclidean geometry. Crucially, Family 4 (Non-Normal Spectrum) introduces dynamic operator properties that reflect how congestion waves diffuse or amplify across directed graphs. Finally, Family 5 (Cross Interactions) computes physically motivated dimensionless ratios (e.g., Spectral Shock Index $N_{\text{veh}} \cdot K(\tilde{A}_w) / C_{\text{tot}}$) that allow gradient-boosted trees to isolate non-linear phase transitions from free-flow to gridlock.
...
\begin{table}[htbp]
\caption{Statistical Correlation of Top Graph-Spectral Descriptors with Normalized Emissions ($y$) vs. Raw Total Emissions ($Y$)}
\label{tab:correlation}
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Descriptor Name} & \textbf{Correlation with $Y$ (Raw $\text{CO}_2$)} & \textbf{Correlation with $y$ (Per-Veh)} \\
\midrule
...
\begin{tabular}{lcccc}
\toprule
\textbf{Hyperparameter} & \textbf{Search Grid} & \textbf{Optimal Value} & \textbf{Test $R^2$ (\%)} & \textbf{CV Std (\%)} \\
\midrule
...
\begin{table}[htbp]
\caption{5-Fold Cross-Validation Metrics on Rebuilt Total $\text{CO}_2$ across 5 Independent Global City Folds}
...
To evaluate model performance under realistic operational conditions, Table~\ref{tab:case_studies} presents a detailed comparative analysis between microscopic SUMO physics simulations and our graph-spectral surrogate across six distinct global urban archetypes.
...
\begin{tabular}{clccl}
\toprule
\textbf{Rank} & \textbf{Feature Name} & \textbf{Mean $|\text{SHAP}|$ (g/veh)} & \textbf{Rel. Importance (\%)} & \textbf{Effect on Emissions ($y$)} \\
\midrule
1 & Electrification Ratio $\text{Fleet}_{\text{elec}}$ & 142.5 & 31.8 & Strongly Negative ($-$) \\
2 & Spectral Impedance $\rho(\tilde{A}_w)$ & 88.4 & 19.7 & Strongly Positive ($+$) \\
3 & Commutator Non-Normality $\Delta(\tilde{A}_0)$ & 54.2 & 12.1 & Positive ($+$) \\
4 & Kreiss Constant $K(\tilde{A}_w)$ & 41.8 & 9.3 & Positive ($+$) \\
5 & Spectral Shock Index & 34.6 & 7.7 & Positive ($+$) \\
6 & Meshedness Coefficient $M$ & 26.1 & 5.8 & Negative ($-$) \\
7 & Speed Limit Variance $\sigma_W^2$ & 19.4 & 4.3 & Positive ($+$) \\
8 & Kato Eigenvalue Derivative $\lambda^{(1)}$ & 16.2 & 3.6 & Positive ($+$) \\
9 & Spectral Gap $\Delta\lambda$ & 13.5 & 3.0 & Negative ($-$) \\
10 & Hardy Resolvent Norm $\|R\|_{H_2}$ & 11.8 & 2.6 & Positive ($+$) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\caption{Barycentric Convex Hull Reconstruction Error vs. Empirical Prediction Error Range}
\label{tab:barycentric}
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Hull Distance $d_{\mathcal{H}}$} & \textbf{Generalization Regime} & \textbf{Active Weights $n_{\text{active}}$} & \textbf{Max Rel. Error (\%)} \\
\midrule
...
$d_{\mathcal{H}} \ge 0.100$ & Out-of-Distribution Warning & $< 4$ or Isolated & $> 10.0\%$ (Flagged) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[htbp]
\caption{End-to-End Computational Latency and Resource Breakdown Across Network Scales}
\label{tab:cold_start}
\centering
\begin{tabular}{lcccccccc}
\toprule
\textbf{Metropolitan Network} & \textbf{Nodes $n$} & \textbf{Edges $m$} & \textbf{OSM Parse} & \textbf{Sparse Eigensolver} & \textbf{Feature Extract} & \textbf{AI Inference} & \textbf{Total Cold-Start} & \textbf{SUMO Sim (1-hr)} \\
\midrule
\textbf{Small (Versailles)} & 2,184 & 4,892 & 1.4 s & 0.6 s & 0.4 s & 5.4 ms & 2.4 s & 1,420 s (23.6 min) \\
\textbf{Medium (Dublin)} & 5,120 & 11,340 & 3.2 s & 1.4 s & 0.8 s & 5.5 ms & 5.4 s & 3,450 s (57.5 min) \\
\textbf{Large (Paris)} & 11,438 & 24,960 & 7.8 s & 3.8 s & 1.6 s & 5.6 ms & 13.2 s & 7,420 s (2.06 h) \\
\textbf{Very Large (Los Angeles)} & 28,640 & 61,200 & 19.5 s & 11.2 s & 3.9 s & 5.8 ms & 34.6 s & 18,360 s (5.10 h) \\
\textbf{Megacity (Tokyo)} & 34,812 & 78,450 & 24.8 s & 15.6 s & 4.8 s & 6.1 ms & 45.2 s & 22,400 s (6.22 h) \\
\textbf{Metropolis ($>100\text{k}$)} & 104,200 & 235,100 & 68.4 s & 48.2 s & 15.4 s & 8.2 ms & 132.0 s & $> 24.0$ h (Est.) \\
\bottomrule
\end{tabular}
\end{table*}
...
To translate these theoretical gains into municipal practice, we integrated the XGBoost surrogate into an interactive digital twin application developed in Python with Streamlit and FastAPI. Municipal traffic engineers can interactively modify ZFE low-emission boundaries, adjust EV fleet adoption sliders, simulate arterial lane closures, and receive updated city-wide $\text{CO}_2$ emission estimates in sub-second response times, enabling real-time multi-criteria urban decarbonization planning.
