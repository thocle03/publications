...
All results reported in this section are obtained using an 80/20 train--test split on the full dataset of 349 SUMO+HBEFA3 simulations, with the test set strictly held out during model training. In addition, we perform 5-fold cross-validation to assess robustness, and a separate zero-shot evaluation on 9 unseen cities excluded from both training and validation.
...
\caption{Computational execution time comparison between microscopic SUMO simulations (seconds, log-scale) and the Graph-Spectral AI surrogate ($5.62$~ms), highlighting massive speedup factors ($>10^6\times$).}
\label{fig:speedup}
\end{figure}
