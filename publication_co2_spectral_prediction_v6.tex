...
\title{Topological Graph-Spectral Machine Learning for Fast Surrogate Prediction of Simulation-Derived Urban $\text{CO}_2$ Emissions: Harnessing Non-Normal Network Physics and Kato Perturbations}
...
{Clerc \MakeLowercase{\textit{et al.}}: Topological Graph-Spectral ML for Fast Surrogate Prediction of Urban $\text{CO}_2$ Emissions}

\maketitle

\begin{abstract}
Microscopic traffic simulators such as SUMO, combined with physical emission models like HBEFA3, provide high-fidelity $\text{CO}_2$ estimates but are computationally prohibitive for real-time urban planning. This paper introduces a Topological Graph-Spectral Surrogate Framework that predicts city-scale $\text{CO}_2$ emissions instantaneously from the directed road network alone. Road graphs are converted into physically weighted impedance matrices, from which we extract non-normal spectral signatures including spectral radius $\rho(A_0)$ and Kreiss-based transient amplification indicators $K(A_0)$ that quantify congestion sensitivity and network memory. Trained on 349 physics-validated SUMO+HBEFA3 simulations across 65 cities worldwide, our vehicle-normalized XGBoost surrogate achieves $R^2 = 89.39\%$ on unitary emissions and $R^2 = 98.95\%$ on reconstructed totals, with strong zero-shot generalization ($R^2 = 88.66\%$) on unseen urban geometries backed by a novel convex barycentric reconstruction diagnostic in $\mathbb{R}^{21}$. Inference executes in under $6$~ms, yielding speedups above $10^6\times$ compared to microscopic simulation, while full cold-start processing for large métropoles completes in under $132$~s. This establishes a mathematically grounded and computationally scalable foundation for real-time digital twins and interactive urban decarbonization tools.
\end{abstract}
...
\end{equation}
where $E_{\text{bidirectional}} \subseteq E$ denotes the set of edges that possess a corresponding reverse edge in the directed graph (i.e., bidirectional street segments). Matrix asymmetry often correlates with non-normality in urban transport networks, but does not strictly imply it ($A A^T \neq A^T A$). We measure the degree of non-normality via the Frobenius norm of the matrix commutator $[A, A^T] = A A^T - A^T A$, a standard metric in non-normal operator theory \cite{Schur1909, Henrici1962}:
\begin{equation}
...
To substitute microscopic simulation, our surrogate regression model utilizes a compact vector $\mathbf{x} \in \mathbb{R}^{47}$ of expert features. These 47 descriptors capture traffic demand, fleet composition, network geometry, and non-normal spectral dynamics, providing a physically grounded representation of congestion sensitivity and shockwave amplification. 

To prevent mathematical ambiguity, we formally distinguish between:
\begin{itemize}
    \item The \emph{unweighted adjacency matrix} $A_0 \in \{0, 1\}^{n \times n}$, where $A_{0, ij} = 1$ if $(v_i, v_j) \in E$ and $0$ otherwise, capturing pure network connectivity.
    \item The \emph{weighted impedance matrix} $A_w \in \mathbb{R}^{n \times n}$, where $A_{w, ij} = \frac{L_{ij}}{W_{ij} \cdot C_{ij}}$ if $(v_i, v_j) \in E$ and $0$ otherwise, capturing physical travel time and capacity dynamics.
\end{itemize}

The 47 descriptors are categorized into 8 thematic families:
\begin{enumerate}
    \item \textbf{Traffic Demand \& Composition (7 features)}: Quantifies the macroscopic volume load and modal fleet proportions exerting kinematic pressure on the network. Includes total vehicles ($N_{\text{veh}}$), simulation duration ($T_{\text{sim}}$), fleet proportions ($\text{ratio}_{\text{car}}, \text{ratio}_{\text{moto}}, \text{ratio}_{\text{truck}}, \text{ratio}_{\text{bus}}$), and relative load ($\text{load}_{\text{rel}} = N_{\text{veh}} / n$).
    \item \textbf{Decoupled Electrification Rates (4 features)}: Captures zero-tailpipe emission penetration per vehicle category to model direct decarbonization. Includes categorical EV penetration percentages ($\text{pct}_{\text{car\_ev}}, \text{pct}_{\text{moto\_ev}}, \text{pct}_{\text{truck\_ev}}, \text{pct}_{\text{bus\_ev}}$).
    \item \textbf{General Topology (10 features)}: Encodes static 2D planar graph geometry, spatial density, and boundary inflow/outflow constraints. Includes node count $n = |V|$, edge count $m = |E|$, spatial density $D = \frac{m}{n(n-1)}$, average degree $\langle k \rangle = \frac{m}{n}$, asymmetry index $\alpha(G) = 1.0 - |E_{\text{bidirectional}}|/|E|$, source count $n_{\text{source}}$ ($k_{\text{in}} = 0$) and its ratio $n_{\text{source}}/n$, sink count $n_{\text{sink}}$ ($k_{\text{out}} = 0$) and its ratio $n_{\text{sink}}/n$, and edge-to-node ratio $m/n$.
    \item \textbf{Unweighted Spectral Metrics (5 features)}: Provides scalar operator-theoretic indicators of global network nervousness, worst-case resolvent bounds, and non-normal transient amplification. Includes unweighted spectral radius $\rho(A_0) = \max_i |\lambda_i(A_0)|$, maximum singular value $\sigma_1(A_0) = \|A_0\|_2$, unweighted Kreiss constant $K(A_0)$, unweighted Hardy $H_2$ norm $\|T_0\|_{H_2}$, and commutator non-normality $\Delta(A_0) = \|A_0 A_0^T - A_0^T A_0\|_F$.
    \item \textbf{Multidimensional Spectral Space of $A_0$ (10 features)}: Captures the multi-scale hierarchy of dominant vibrational modes and alternative routing redundancies. Includes Top-5 eigenvalue magnitudes sorted such that $|\lambda_1| \ge |\lambda_2| \ge \dots \ge |\lambda_5|$ (with $|\lambda_1| = \rho(A_0)$), and Top-5 singular values sorted such that $\sigma_1 \ge \sigma_2 \ge \dots \ge \sigma_5$.
    \item \textbf{Physically Weighted Spectral Metrics (3 features)}: Integrates physical link travel times and capacity constraints into spectral decay rates. Includes impedance-weighted spectral radius $\rho(A_w)$, weighted maximum singular value $\sigma_1(A_w)$, and weighted Hardy $H_2$ norm $\|T_w\|_{H_2}$.
    \item \textbf{Interaction Descriptors (2 features)}: Models non-linear coupled risks between physical demand loading and spectral vulnerability bounds. Includes non-linear coupled risk metrics ($\text{risk}_{\text{Kreiss}} = K(A_0) \times \text{load}_{\text{rel}}$ and $\text{risk}_{\text{spectral}} = \rho(A_0) \times \text{load}_{\text{rel}}$).
    \item \textbf{Geographical Region One-Hot (6 features)}: Serves as a contextual proxy for regional driving styles, fleet age distributions, and topography via binary encoding of continent of origin.
\end{enumerate}
Together, these descriptors provide a compact yet expressive representation of the urban network, enabling the surrogate model to capture both structural constraints and dynamic congestion phenomena.
...
\end{equation}
where $N$ is the number of training samples, $y_i$ is the target value (ground-truth per-vehicle $\text{CO}_2$ emissions), $\hat{y}_i^{(t-1)}$ is the prediction of the ensemble model at step $t-1$, $f_t(\mathbf{x}_i)$ is the new regression tree (weak learner) added at step $t$ for input feature vector $\mathbf{x}_i$, and $l(\cdot)$ is the differentiable loss function (mean squared error). The term $\Omega(f_t)$ is the tree complexity regularizer defined as:
\begin{equation}
\Omega(f_t) = \gamma T + \frac{1}{2}\lambda \sum_{j=1}^T w_j^2 + \alpha \sum_{j=1}^T |w_j|,
\end{equation}
where $T$ is the number of leaves in tree $f_t$, $w_j$ is the output score (weight) of leaf node $j$, $\gamma$ is the complexity penalty parameter that penalizes leaf count (pre-pruning threshold), and $\lambda$ and $\alpha$ are the L2 and L1 regularization coefficients on the leaf weights, respectively. Using a second-order Taylor expansion to approximate the objective:
\begin{equation}
\mathcal{L}^{(t)} \approx \sum_{i=1}^N \left[ g_i f_t(\mathbf{x}_i) + \frac{1}{2} h_i f_t^2(\mathbf{x}_i) \right] + \gamma T + \frac{1}{2}\lambda \sum_{j=1}^T w_j^2 + \alpha \sum_{j=1}^T |w_j|,
\end{equation}
where $g_i = \partial_{\hat{y}^{(t-1)}} l(y_i, \hat{y}^{(t-1)})$ and $h_i = \partial^2_{\hat{y}^{(t-1)}} l(y_i, \hat{y}^{(t-1)})$ represent the first-order gradient and second-order Hessian of the loss function, respectively. The second-order Hessian $h_i$ enables XGBoost to capture abrupt non-linear phase transitions (e.g., sudden gridlock onset) far more effectively than standard first-order algorithms. Hyperparameters are tuned via early stopping ($n_{\text{estimators}} = 500$, stopping at $\sim 300$ trees, $\text{max\_depth} = 8$, $\text{learning\_rate} = 0.05$, $\text{subsample} = 0.8$, $\text{colsample\_bytree} = 0.8$).
...
\begin{itemize}
    \item \textbf{XGBoost vs. Linear Baselines (Group 1)}: Linear models (OLS, Ridge, Lasso) yield lower performance ($R^2 = 86.79\% - 94.42\%$) because they assume a linear relationship, failing to capture abrupt traffic state transitions (such as queue spillbacks) and resulting in double the RMSE of XGBoost ($13,240 - 20,367$~kg).
    \item \textbf{XGBoost vs. Kernel Methods (Group 2)}: SVR with an RBF kernel achieves $96.73\%$ total $R^2$ but suffers from elevated RMSE ($10,134$~kg) because global kernel smoothing fails to fit local transient traffic spikes.
    \item \textbf{XGBoost vs. Neural Architectures (Group 3)}: MLP neural networks collapse completely ($R^2 = -37.75\%$) due to overfitting on tabular variables in moderate datasets, while GCNs ($R^2 = 45.92\%$ unitary) are restricted to local neighborhood convolutions, failing to capture global asymptotic non-normal resolvent bounds ($K(A_0), H_2$).
    \item \textbf{XGBoost vs. Tree Ensembles (Group 4)}: Random Forest and Extra Trees rely on tree bagging, which smooths out sharp local congestion peaks. Standard GBDT is restricted to first-order gradients, whereas XGBoost computes second-order Taylor expansion loss gradients ($h_i$) and applies L1/L2 leaf regularization, providing superior stability on traffic discontinuities.
\end{itemize}
...
To ensure methodological rigor and evaluate both in-distribution precision and out-of-distribution transferability, our experimental validation is organized into a \textbf{Three-Tier Evaluation Protocol}:
\begin{enumerate}
    \item \textbf{Tier 1: Model Training Phase ($80\%$ of Multi-City Corpus, $279$ Simulations)}: Gradient boosted decision trees are trained on normalized per-vehicle emissions across $65$ global cities spanning diverse topologies.
    \item \textbf{Tier 2: Internal Testing \& Robustness Phase ($20\%$ Holdout, $70$ Simulations + $5$-Fold Cross-Validation)}: Evaluates predictive accuracy on unseen traffic scenarios within the known 65-city geometry pool ($R^2 = 89.39\%$ unitary, $R^2 = 98.95\%$ rebuilt total), accompanied by a 5-fold cross-validation to verify statistical stability ($R^2 = 97.99 \pm 1.00\%$).
    \item \textbf{Tier 3: External Zero-Shot Generalization Phase ($9$ Unseen Cities, $18$ Simulations)}: Evaluates true out-of-distribution transfer on completely unobserved urban road networks excluded from both training and validation ($R^2 = 88.66\%$), diagnostically backed by convex barycentric reconstruction in $\mathbb{R}^{21}$.
\end{enumerate}
...
To measure the exact incremental contribution of individual spectral sub-components—specifically Kreiss constant $K(A_0)$, Hardy norm $H_2(A_0)$, non-normalness $\Delta(A_0)$, and the singular value spectrum ($\sigma_1, \dots, \sigma_5$)—we benchmarked 8 feature subsets (Table~\ref{tab:fine_ablation}).
...
\midrule
\textbf{Full Model (Reference)} & \textbf{44} & \textbf{98.64 \%} & \textbf{6,543.0 kg} & \textbf{3,875.4 kg} & \textbf{25.30 \%} & \textbf{0.0 kg (1.00$\times$)} \\
Full WITHOUT Singular Values & 32 & 98.24 \% & 7,426.1 kg & 4,242.1 kg & 26.02 \% & +883.1 kg ($1.13\times$) \\
Full WITHOUT Hardy $H_2$ Norm & 42 & 97.78 \% & 8,346.1 kg & 4,037.2 kg & 25.54 \% & +1,803.1 kg ($1.28\times$) \\
Full WITHOUT Non-Normalness $\Delta(A_0)$ & 42 & 97.68 \% & 8,536.0 kg & 4,266.6 kg & 25.79 \% & +1,993.0 kg ($1.30\times$) \\
Full WITHOUT Kreiss Constant $K(A_0)$ & 43 & 96.88 \% & 9,890.7 kg & 4,456.5 kg & 23.38 \% & \textbf{+3,347.7 kg ($1.51\times$)} \\
\midrule
\textbf{Spectral ONLY (Spectral Suite Alone)} & \textbf{18} & \textbf{94.83 \%} & \textbf{12,742.1 kg} & \textbf{7,537.5 kg} & \textbf{57.39 \%} & +6,199.1 kg ($1.95\times$) \\
Traffic ONLY (Volume \& Fleet Composition) & 11 & 92.52 \% & 15,320.8 kg & 6,875.3 kg & 34.13 \% & +8,777.8 kg ($2.34\times$) \\
Topology ONLY (Nodes, Edges, Density) & 9 & 90.05 \% & 17,674.8 kg & 9,006.6 kg & 57.64 \% & +11,131.8 kg ($2.70\times$) \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{enumerate}
    ...
    \item \textbf{Spectral Suite Dominance}: The feature subset containing only graph-spectral metrics (\texttt{Spectral ONLY}, 18 features) achieves a high total $R^2 = 94.83\%$, outperforming both the \texttt{Traffic ONLY} subset ($R^2 = 92.52\%$, 11 features) and the \texttt{Topology ONLY} subset ($R^2 = 90.05\%$, 9 features) when operating in isolation. This demonstrates that spectral signatures, by capturing the mathematical properties of wave propagation and network bottlenecks, are intrinsically more informative of congestion patterns than simple traffic volume indicators or raw graph counts (nodes, edges).
...
\midrule
1 & Bus EV Penetration ($\text{pct}_{\text{bus\_ev}}$) & 0.1789 & \textbf{Strong Reduction}: Public transit EV adoption lowers per-vehicle footprint. \\
2 & Regional Proxy: South America & 0.1044 & \textbf{Increase}: Captures mountainous terrain forcing high engine load. \\
3 & Commutator Non-Normality ($\Delta(A_0)$) & 0.0700 & \textbf{Strong Increase}: Non-normal matrix asymmetry drives stop-and-go idling. \\
4 & Passenger EV Penetration ($\text{pct}_{\text{car\_ev}}$) & 0.0514 & \textbf{Reduction}: Passenger EV adoption reduces thermal baseline emissions. \\
5 & 4th Eigenvalue Magnitude ($|\lambda_4|$) & 0.0389 & \textbf{Modulation}: Spectral eigenvalue mode 4 quantifies alternative detour capacity. \\
6 & Unweighted Kreiss Constant ($K(A_0)$) & 0.0338 & \textbf{Increase}: Higher Kreiss bounds amplify transient shockwave risk. \\
7 & Weighted Spectral Radius ($\rho(A_w)$) & 0.0335 & \textbf{Increase}: Elevated transit impedance lengthens trip duration. \\
8 & Relative Demand Load ($\text{load}_{\text{rel}}$) & 0.0273 & \textbf{Increase}: Higher vehicle-per-node ratios drive intersection saturation. \\
9 & Weighted Hardy $H_2$ Norm ($\|T_w\|_{H_2}$) & 0.0234 & \textbf{Increase}: Higher $H_2$ norms delay post-peak queue evacuation. \\
10 & Average Out-Degree ($\langle k \rangle$) & 0.0231 & \textbf{Reduction}: Higher intersection degree improves routing flexibility. \\
\bottomrule
\end{tabular}
\end{table*}

\begin{table}[htbp]
\caption{Top 15 Feature Importances (XGBoost Surrogate Model Normalized $\text{CO}_2$/veh)}
...
\midrule
1 & Total Vehicle Count ($N_{\text{veh}}$) & Traffic Volume & 11.58 \% \\
2 & Simulation Duration ($T_{\text{sim}}$) & Traffic Duration & 6.09 \% \\
3 & Motorcycle EV Penetration ($\text{pct}_{\text{moto\_ev}}$) & Electrification & 5.70 \% \\
4 & Edge-to-Node Ratio ($m/n$) & Topology & 5.61 \% \\
5 & Truck Proportion ($\text{ratio}_{\text{truck}}$) & Fleet Composition & 4.71 \% \\
6 & Commutator Non-Normality ($\Delta(A_0)$) & Spectral & 4.48 \% \\
7 & Motorcycle Proportion ($\text{ratio}_{\text{moto}}$) & Fleet Composition & 4.06 \% \\
8 & Spectral Interaction Risk ($\text{risk}_{\text{spectral}}$) & Interaction & 3.98 \% \\
9 & Source Node Count ($n_{\text{source}}$) & Topology & 3.77 \% \\
10 & Graph Asymmetry Index ($\alpha(G)$) & Topology & 3.75 \% \\
11 & Unweighted Kreiss Constant ($K(A_0)$) & Spectral & 3.67 \% \\
12 & Relative Demand Load ($\text{load}_{\text{rel}}$) & Interaction & 3.48 \% \\
13 & Spatial Density ($D$) & Topology & 3.44 \% \\
14 & Weighted Hardy $H_2$ Norm ($\|T_w\|_{H_2}$) & Spectral & 3.23 \% \\
15 & Edge Count ($m$) & Topology & 2.51 \% \\
\bottomrule
\end{tabular}
}
\end{table}
...
\textit{Zero-Shot Cross-City Generalization Protocol}: To evaluate the ability of the surrogate model to generalize beyond the cities used during training, we perform a dedicated zero-shot experiment. In this setting, an entire subset of cities is completely excluded from both the training and validation splits. The model is trained on 349 SUMO+HBEFA3 simulations covering 65 cities, using an 80/20 train--test split strictly applied within this set. A separate group of 9 additional cities---never seen during training, validation, or hyperparameter tuning---is then used to assess zero-shot performance. Because the model has no prior exposure to the topologies, spectral signatures, or traffic compositions of these cities, this evaluation measures true out-of-distribution generalization. The resulting zero-shot $R^2 = 88.66\%$ demonstrates that the spectral surrogate captures transferable structural and dynamical properties of urban networks, enabling accurate $\text{CO}_2$ prediction even on previously unseen geometries.
To test true out-of-sample generalization, we evaluated the model on 9 completely unseen global cities (Nelson, Maseru, Pamplona, Essaouira, Siem Reap, Nara, Galveston, Guanajuato, Colmar) across 18 realistic moderate-to-heavy traffic scenarios. The model was never exposed to these geometries during training or feature extraction. We couple this performance evaluation with a novel geometric diagnostic—the barycentric reconstruction error—to determine whether the prediction limits are structurally bounded by the topological convex hull of our training corpus.
...
\textit{Barycentric Reconstruction Error Diagnostics}: To determine whether prediction errors stem from model extrapolation or missing physical dimensions, we compute the convex barycentric reconstruction error $E_{\text{bary}}$ for each unseen target city:
\begin{equation}
E_{\text{bary}} = \left\| \mathbf{x}_{\text{target}} - \sum_{j=1}^k \alpha_j \mathbf{x}_j \right\|_2^2,
\end{equation}
where $\mathbf{x}_{\text{target}} \in \mathbb{R}^{d}$ ($d=21$) represents the normalized morpho-spectral feature vector of the unseen target city, and $\{\mathbf{x}_1, \dots, \mathbf{x}_k\}$ represents the set of morpho-spectral feature vectors of the $k=65$ training cities. The weights $\alpha_j \ge 0$ (for $j = 1, \dots, k$) are the convex projection coefficients obtained by solving the quadratic optimization problem:
\begin{align}
\min_{\boldsymbol{\alpha}} \ & \left\| \mathbf{x}_{\text{target}} - \sum_{j=1}^k \alpha_j \mathbf{x}_j \right\|_2^2 \\
\text{s.t.} \ & \sum_{j=1}^k \alpha_j = 1 \quad \text{and} \quad \alpha_j \ge 0, \ \forall j \in \{1, \dots, k\}.
\end{align}
This error measures the Euclidean distance in the $d$-dimensional normalized morpho-spectral space between the target city and its optimal projection on the convex hull of the training city topologies. Table~\ref{tab:barycentric} lists this error for each city.
...
\caption{Computational execution time comparison: Microscopic SUMO physics simulation (exploding non-linearly with vehicle volume) versus the Graph-Spectral AI surrogate (constant $5.62$~ms inference, $\mathcal{O}(1)$), demonstrating an acceleration gap exceeding $1,000,000\times$.}
\label{fig:speedup}
\end{figure}
...
\caption{Cold-start pipeline scalability across European métropoles: total processing time (download, graph build, spectral extraction, inference) scales linearly with raw OSM network node count.}
...
%-------------------------------------------------------------------------------
\section{Discussion \& Operational Digital Twin Deployment}

\subsection{Interactive Digital Twin Deployment}
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig_digital_twin.png}
\caption{Operational Streamlit Digital Twin Dashboard: real-time scenario explorer featuring interactive traffic demand sliders, EV fleet penetration controls, spectral vulnerability metrics ($SV, K(A_0)$), and instantaneous $\text{CO}_2$ emission spatial overlays.}
...
\begin{enumerate}
    ...
    \begin{equation}
    SV = 0.4 \times \tanh\left( \frac{NN_{\text{rel}}}{2.0} \right) + 0.6 \times \tanh\left( \frac{K(A_0)}{25.0} \right),
\end{equation}
where $NN_{\text{rel}} = \Delta(A_0)/n$ represents the relative non-normality index (the commutator norm scaled by the node dimension $n = |V|$ to enable cross-city comparisons), and $K(A_0)$ is the unweighted Kreiss constant. The weights ($0.4$ and $0.6$) and scaling denominators ($2.0$ and $25.0$) are calibrated empirical coefficients based on our feature correlation analysis (Table~\ref{tab:spectral_correlations}), where the global resolvent bound $K(A_0)$ exhibits a stronger correlation with cascading queue spillbacks than local commutator non-normality. The hyperbolic tangent function $\tanh(\cdot)$ acts as a non-linear mapping that projects unbounded structural metrics into a normalized vulnerability range $[0, 1)$, while implementing a saturation effect: once a network's instability metrics exceed critical thresholds (e.g., $K(A_0) > 50$), the structural vulnerability saturates near 1.0, alerting operators of imminent gridlock susceptibility.
...
%-------------------------------------------------------------------------------
\section{Conclusion}
This paper presented a Topological Graph-Spectral Machine Learning framework for fast surrogate prediction of simulation-derived urban $\text{CO}_2$ emissions. By modeling road networks as asymmetric physically weighted impedance matrices and extracting non-normal operator signatures—including spectral radius $\rho(A_0)$, Kreiss constants $K(A_0)$, commutator non-normality $\Delta(A_0)$, and Hardy $H_2$ norms—we successfully bypassed the severe computational limitations of traditional microscopic multi-agent physics simulators. (Note that the Hardy $H_\infty$ norm and Kato perturbation derivatives, while theoretically crucial for bounding worst-case dynamics and guiding control optimization, are excluded from the training feature vector to maintain real-time feature extraction compliance). 
...
Despite these strong capabilities, three primary limitations outline clear avenues for future research:
\begin{enumerate}
    \item \textbf{Planar 2D Topologies vs. 3D Terrain}: Road graphs are extracted from 2D OpenStreetMap cartography, omitting vertical road gradients (slopes increasing heavy vehicle fuel consumption) and 3D subterranean bypass tunnels (as observed in Guanajuato). Integrating Digital Elevation Models (DEM) directly into edge impedance weights $A_w$ will bridge this topographical gap.
    \item \textbf{Microscopic Signal Cycles and Filtering Friction}: The surrogate operates at mesoscopic graph-spectral scale, aggregating intersection capacities without explicitly resolving adaptive traffic light phase timings or chaotic motorcycle lane-filtering dynamics (which caused mild underestimations in dense Asian topologies like Siem Reap).
    \item \textbf{Continuous Live Sensor Assimilation}: Ground-truth emissions stem exclusively from microscopic SUMO+HBEFA3 simulations. Operational municipal deployments will benefit from continuous Bayesian fine-tuning and calibration against empirical IoT air quality sensor networks and floating-car GPS probes.
\end{enumerate}
