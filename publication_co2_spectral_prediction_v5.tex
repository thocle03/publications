...
To substitute microscopic simulation, our surrogate regression model utilizes a compact vector $x \in \mathbb{R}^{47}$ of expert features. To prevent any mathematical ambiguity, we formally distinguish between:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{General Topology (10 features)}: 
    \begin{itemize}
        \item Node count $n = |V|$ and edge count $m = |E|$.
        \item Spatial density $D = \frac{m}{n(n-1)}$, measuring the ratio of actual links to the maximum possible directed connections.
        \item Average degree $\langle k \rangle = \frac{m}{n}$, representing average in-degree and out-degree.
        \item Asymmetry index $\alpha(G) = 1.0 - |E_{\text{bidirectional}}|/|E|$, where $E_{\text{bidirectional}}$ is the count of two-way links.
        \item Source count $n_{\text{source}}$ (nodes with in-degree $k_{\text{in}} = 0$) and its ratio $n_{\text{source}}/n$.
        \item Sink count $n_{\text{sink}}$ (nodes with out-degree $k_{\text{out}} = 0$) and its ratio $n_{\text{sink}}/n$.
        \item Edge-to-node ratio $\frac{m}{n}$ (identical to average degree $\langle k \rangle$).
    \end{itemize}
    \item \textbf{Unweighted Spectral Metrics (5 features)}:
    \begin{itemize}
        \item Unweighted spectral radius $\rho(A_0) = \max_i |\lambda_i(A_0)|$.
        \item Maximum singular value $\sigma_1(A_0) = \|A_0\|_2$ (spectral norm), identical to the first value in Family 5.
        \item Unweighted Kreiss constant $K(A_0)$.
        \item Unweighted Hardy $H_2$ norm $\|T_0\|_{H_2}$ of transfer function $T_0(z) = (zI - A_0)^{-1}$.
        \item Commutator non-normality $\Delta(A_0) = \|A_0 A_0^T - A_0^T A_0\|_F$.
    \end{itemize}
    \item \textbf{Multidimensional Spectral Space of $A_0$ (10 features)}: 
    \begin{itemize}
        \item Top-5 eigenvalues magnitudes sorted such that $|\lambda_1| \ge |\lambda_2| \ge \dots \ge |\lambda_5|$. Note that $|\lambda_1| = \rho(A_0)$. Only the real magnitudes are utilized, preventing complex number encoding issues in the XGBoost decision trees.
        \item Top-5 singular values sorted such that $\sigma_1 \ge \sigma_2 \ge \dots \ge \sigma_5$. Note that $\sigma_1$ is identical to $\sigma_1(A_0)$ in Family 4.
    \end{itemize}
    \item \textbf{Physically Weighted Spectral Metrics (3 features)}:
    \begin{itemize}
        \item Impedance-weighted spectral radius $\rho(A_w)$.
        \item Weighted maximum singular value $\sigma_1(A_w)$.
        \item Weighted Hardy $H_2$ norm $\|T_w\|_{H_2}$ of transfer function $T_w(z) = (zI - A_w)^{-1}$.
    \end{itemize}
    \item \textbf{Interaction Descriptors (2 features)}: Non-linear coupled risk metrics ($\text{risk}_{\text{Kreiss}} = K(A_0) \times \text{load}_{\text{rel}}$ and $\text{risk}_{\text{spectral}} = \rho(A_0) \times \text{load}_{\text{rel}}$).
